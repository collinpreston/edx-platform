...
ARG CONFIG_ROOT='/edx/etc'
ARG LMS_CFG="$CONFIG_ROOT/lms.yml"
ARG CMS_CFG="$CONFIG_ROOT/cms.yml"
...
RUN set -eux; \
    ...
    # Install development requirements only If DEVELOPMENT argument is set to true at build time.
    # Link configuration YAMLs and set EDX_PLATFORM_SETTINGS.
    if $DEVELOPMENT ; then \
        pip install -r requirements/edx/development.txt && \
        ln -s "$(pwd)/lms/envs/devstack-experimental.yml" "$LMS_CFG" && \
        ln -s "$(pwd)/cms/envs/devstack-experimental.yml" "$CMS_CFG" && \
